...
import { 
    ...
    Activity, AlertTriangle, ArrowRight, RadioReceiver, ScanEye
} from 'lucide-react';
...
const TransportDashboard = () => {
    ...
    // ⚠️ FAKE DATA: This will be replaced by real Socket.io data later
    const fleetStats = [
        { label: 'Active Buses', value: '08', icon: <Bus size={28} />, color: 'text-emerald-500', bg: 'bg-emerald-50' },
        { label: 'Students in Transit', value: '342', icon: <Users size={28} />, color: 'text-[#42A5F5]', bg: 'bg-blue-50' },
        { label: 'SOS / Alerts', value: '00', icon: <AlertTriangle size={28} />, color: 'text-amber-500', bg: 'bg-amber-50' },
    ];

    const activeRoutes = [
        { id: 'BUS-01', route: 'North Sector A', driver: 'Ramesh Singh', eta: '10 mins', status: 'In Transit', passengers: 45, progress: 75 },
        { id: 'BUS-04', route: 'West Sector B', driver: 'Suresh Kumar', eta: 'Arrived', status: 'Completed', passengers: 52, progress: 100 },
        { id: 'BUS-07', route: 'South City Core', driver: 'Vikram Das', eta: '5 mins', status: 'In Transit', passengers: 38, progress: 85 },
    ];

    return (
        // Removed 'fixed inset-0' so it flows naturally under your Navbar
        <div className="bg-[#F8FAFC] pb-24 font-sans italic text-slate-800 px-5 pt-6 relative z-10 max-w-6xl mx-auto">
            
            {/* 🔥 SLEEK SUB-HEADER (Fits perfectly below Navbar) 🔥 */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 px-2">
                <div>
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-slate-800">Fleet Command</h1>
                    <p className="text-[12px] font-black text-[#42A5F5] uppercase tracking-widest flex items-center gap-2 mt-1">
                        <RadioReceiver size={16} className="animate-pulse" /> Live Tracking Matrix
                    </p>
                </div>
                <div className="text-left md:text-right bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div className="space-y-6">
                ...
                {/* 🔥 LIVE RADAR MAP SIMULATOR (No Google Maps yet) 🔥 */}
                <motion.div 
                ...
                >
                    ...
                    <div className="relative z-10 flex justify-between items-center mb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Sector Overview</h2>
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">OpenStreetMap Protocol (Standby)</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-full backdrop-blur-md">
                            ...
                            <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">GPS Synced</span>
                        </div>
                    </div>
                    ...
                    <div className="h-72 md:h-96 w-full bg-slate-800/50 rounded-[2rem] border border-slate-700 relative overflow-hidden flex items-center justify-center shadow-inner">
                        ...
                        <div className="absolute bottom-6 left-0 right-0 text-center">
                            <p className="text-slate-500 font-bold uppercase tracking-widest text-[12px] bg-slate-900/80 inline-block px-4 py-2 rounded-full backdrop-blur-md">
                                Awaiting WebSocket Coordinates...
                            </p>
                        </div>
                    </div>
                </motion.div>
                ...
                >
                    <div className="flex justify-between items-center mb-8 px-2">
                        <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Active Transits</h2>
                        ...
                        {activeRoutes.map((bus, i) => (
                            <div key={i} className="p-6 bg-slate-50 border border-slate-100 rounded-[2rem] flex flex-col md:flex-row md:items-center justify-between gap-5 hover:border-[#42A5F5] transition-colors group">
                                <div className="flex items-center gap-6">
                                    <div className={`p-4 rounded-[1.5rem] shadow-sm ${bus.status === 'Completed' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-[#42A5F5]'}`}>
                                        <Bus size={24} />
                                    </div>
                                    <div>
                                        <h3 className="font-black text-slate-800 uppercase text-[17px] tracking-tight flex items-center gap-3">
                                            {bus.id} 
                                            <span className={`text-[9px] px-3 py-1 rounded-full tracking-widest ${bus.status === 'Completed' ? 'bg-emerald-50 text-emerald-500 border border-emerald-200' : 'bg-amber-50 text-amber-500 border border-amber-200'}`}>
                                                {bus.status}
                                            </span>
                                        </h3>
                                ...
                                <div className="flex items-center gap-8 md:gap-12 bg-white px-6 py-4 rounded-[1.5rem] shadow-sm border border-slate-100">
                                    ...
                                    <div className="text-center hidden sm:block">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Onboard</p>
